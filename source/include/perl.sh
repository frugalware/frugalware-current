...
[ -z "$_F_perl_no_up2date" ] && up2date="lynx -read_timeout=280 -dump -nolist 'http://search.cpan.org/search?query="$_F_perl_name"&mode=module'|grep -m1 "$_F_perl_sourcename-"|sed -e 's/.*"$_F_perl_sourcename"-\(.*\) .*/\1/' -e 's/ .*//'"
