...
int main(int argc, char **argv)
{
	...
	for(i=alpm_db_getpkgcache(repo); i; i=alpm_list_next(i))
	{
		...
		if(strcmp(group, "base") && strcmp(group, "apps") && strcmp(group, "lib") && strcmp(group, "multimedia") && strcmp(group, "network") && strcmp(group, "devel"))
			continue;
			...
			for(k=alpm_db_getpkgcache(repo); !found&&k; k=alpm_list_next(k))
			{
				...
				if(!strcmp(alpm_pkg_getinfo(p, PM_PKG_NAME), str))
				{
					...
					for(l=alpm_pkg_getinfo(p, PM_PKG_GROUPS); !found&&l; l=alpm_list_next(l))
					{
						...
						if(!strcmp(g, "x11") || !strcmp(g, "xlib") || !strcmp(g, "xapps") || !strcmp(g, "xmultimedia") || !strcmp(g, "xfce4") || !strcmp(g, "gnome") || !strcmp(g, "kde"))
							buggy=1;
					}
					if(!buggy)
						found=1;
